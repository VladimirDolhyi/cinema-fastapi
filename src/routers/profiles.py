...
from src.database import get_db, User, UserGroupEnum, UserProfile
...
import src.validation.profile as validation
...
def create_profile(
    ...
    request_user = db.query(User).filter(User.id == token_user_id).first()
